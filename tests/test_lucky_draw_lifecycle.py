...
def state():
    ...
    state.lucky_draw_eligible_guests = [
        make_guest("G001", "Alice"),
        make_guest("G002", "Bob"),
        make_guest("G003", "Charlie"),
        make_guest("G004", "David"),
    ]

    state.current_prizes = [
        make_prize("Grand Prize", "RM20,000", rank=1),
        make_prize("Second Prize", "RM10,000", rank=2),
        make_prize("Third Prize", "RM5,000", rank=3),
    ]
...
def test_absent_candidate_is_excluded_from_future_draws(state):
    ...
    # The lifecycle's exclusion operation.
    state.excluded_guest_ids.append(
        str(candidate["guest_id"])
    )
...
def test_redraw_keeps_same_prize(state):
    ...
    original_prize = state.current_prizes[
        state.current_prize_index
    ]
    ...
    assert (
        state.current_prizes[state.current_prize_index]
        == original_prize
    )
...
def test_confirmed_winner_is_removed_from_eligible_pool(state):
    ...
    state.lucky_draw_eligible_guests = [
        guest
        for guest in state.lucky_draw_eligible_guests
        if str(guest["guest_id"])
        != str(winner["guest_id"])
    ]
    ...
    remaining_ids = {
        str(g["guest_id"])
        for g in state.lucky_draw_eligible_guests
    }
...
def test_confirming_prize_does_not_change_prize_until_next_prize(state):
    ...
    # Confirmation itself must not implicitly advance the prize.
    state.draw_status = "CONFIRMED"

    assert state.current_prize_index == 0
    assert (
        state.current_prizes[0]["name"]
        == "Grand Prize"
    )
...
def test_next_prize_does_not_skip_prizes(state):
    ...
    assert [
        prize["name"]
        for prize in state.current_prizes
    ] == [
        "Grand Prize",
        "Second Prize",
        "Third Prize",
...
def test_reset_draw_lifecycle_clears_candidate_and_redraw_state(state):
    ...
    state.pending_candidate = make_guest(
        "G002",
        "Bob",
    )
...
def test_grand_prize_is_rank_one(state):
    ranked = sorted(
        state.current_prizes,
        key=lambda prize: int(prize["rank"]),
    )
...
def test_redraw_excludes_missing_guest(state):
    ...
    state.lucky_draw_eligible_guests = [
        {"guest_id": "G001", "name": "Alice"},
        {"guest_id": "G002", "name": "Bob"},
        {"guest_id": "G003", "name": "Charlie"},
    ]
