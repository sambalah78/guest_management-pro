...
class EmailState(rx.State):
    ...
    async def send_guest_email(self):
        ...
        if not self.selected_guest_for_email:
            yield rx.toast.error("No guest selected.")
            return
        ...
        try:
            ...
            if not guest_id:
                raise ValueError("Guest ID is missing.")

            if not email:
                raise ValueError("No email address for this guest.")

            force = bool(guest.get("email_sent"))
            ...
            # Refresh the guest table immediately.  This does NOT mark
            # the guest as sent prematurely; email_sent becomes True only
            # after the worker/SendGrid acceptance path updates the DB.
            try:
                from guest_management.state.guest_state import GuestState

                guest_state = await self.get_state(GuestState)
                async for result in guest_state.load_guests():
                    yield result
            except Exception:
                logger.exception(
                    "Unable to refresh guest table after queueing email"
                )

            yield rx.toast.success(self.email_message)

        except EventLahError as exc:
            message = getattr(exc, "message", str(exc))
            self.email_error = str(message)
            yield rx.toast.error(self.email_error)

        except Exception as exc:
            logger.exception("Unable to queue guest email")
            self.email_error = (
                "Unable to queue email. Please try again."
            )
            logger.debug("Guest email queue error: %s", exc)
            yield rx.toast.error(self.email_error)
    ...
    async def send_guest_email_for_guest(self, guest: dict):
        """Send one guest's invitation directly from the dashboard row.

        This bypasses the confirmation dialog while reusing the same
        production queue/validation/send path as the email dialog.
        """
        self.selected_guest_for_email = dict(guest or {})
        self.current_event_id = str(self._event_id_from_url())
        self.email_dialog_open = False

        async for result in self.send_guest_email():
            yield result
    ...
    async def refresh_email_management(self):
        ...
        try:
            ...
            # Keep the dashboard guest table synchronized with
            # guests.email_sent after the background worker updates it.
            try:
                from guest_management.state.guest_state import GuestState
                ...
                logger.exception(
                    "Unable to refresh guest table with email status"
                )
